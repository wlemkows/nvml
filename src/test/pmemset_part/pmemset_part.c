		...
		char *argv[])
{
	...
	ut_create_map_config(&map_cfg, set, 0, 64 * 1024, src);

	ret = pmemset_map(map_cfg, NULL, &desc);
	UT_PMEMSET_EXPECT_RETURN(ret, 0);
		...
		char *argv[])
{
	...
	ret = pmemset_map(map_cfg, NULL, NULL);
	UT_PMEMSET_EXPECT_RETURN(ret, 0);
		...
		char *argv[])
{
	...
	UT_PMEMSET_EXPECT_RETURN(ret, 0);
	pmemset_map_config_set_offset(map_cfg, (size_t)(INT64_MAX) + 1);
	pmemset_map_config_set_length(map_cfg, 64 * 1024);
	UT_ASSERTne(map_cfg, NULL);
		...
		char *argv[])
{
	...
	ut_create_map_config(&map_cfg, set, 0, 0, src);
		...
		char *argv[])
{
	...
	ut_create_map_config(&map_cfg, set, 0, part_size, src);
		...
		char *argv[])
{
	...
	ut_create_map_config(&first_map_cfg, set, 0, first_part_size, src);

	ret = pmemset_map(first_map_cfg, NULL, NULL);
	UT_PMEMSET_EXPECT_RETURN(ret, 0);

	ut_create_map_config(&second_map_cfg, set, 0, second_part_size, src);

	ret = pmemset_map(second_map_cfg, NULL, NULL);
	UT_PMEMSET_EXPECT_RETURN(ret, 0);
		...
		char *argv[])
{
	...
	ut_create_map_config(&first_map_cfg, set, 0, part_size_first, src);

	ret = pmemset_map(first_map_cfg, NULL, NULL);
	UT_PMEMSET_EXPECT_RETURN(ret, 0);

	ut_create_map_config(&second_map_cfg, set, 0, part_size_second, src);
		...
		char *argv[])
{
	...
	for (int i = 0; i < nmaps; i++) {
		ut_create_map_config(&map_cfg, set, 0, part_size / 2, src);
		ret = pmemset_map(map_cfg, NULL, &desc);
		UT_PMEMSET_EXPECT_RETURN(ret, 0);
		...
		char *argv[])
{
	...
	ut_create_map_config(&map_cfg, set, 0, SIZE_MAX, src);
